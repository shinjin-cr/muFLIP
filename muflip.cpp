...
template <class T> void print(T *g, int M, int N) { for (int j=0; j<N; ++j) { for (int i=0;i<M; ++i) cout << g[i+j*M] << " "; cout << endl; } }
...
void particles2grid() {
    ...
    for (int k=0; k<PN; ++k) {
        ...
        cout << format("p:(%1%,%2%)\t u corner:(%3%,%4%)\t v corner:(%5%,%6%)\n") 
            % px[k] % py[k] % i % fj % fi % j;
        flags[i+j*N] = FLUID;
        FORCORNERS(i,fj) {
...
void pressureSolve() {
    ...
    cout << "flags:\n"; print(flags,N,N);
    //FORCCIN {
    //    p[i+j*N] = (rand()%200-100)/100.0;
    //    flags[i+j*N] = 1;
    //}
    //for (int k=0; k<N; ++k) {
    //    p[k+(N-2)*N] = p[k+(N-3)*N] = 0;
    //    flags[k+(N-2)*N] = flags[k+(N-3)*N] = 0;
    //}

    //FORCCIN {
    //    double l=i>1?-1:0, r=i<N-2?-1:0, b=j>1?-1:0, t=j<N-2?-1:0;
    //    double c=-(l+r+b+t);
    //    divu[i+j*N] = c*p[i+j*N]+l*p[i-1+j*N]+r*p[i+1+j*N]+b*p[i+j*N-N]+t*p[i+j*N+N];
    //}
    ...
    //cout << "p:\n"; print(p,N,N);
    //printU();
    //FORCC { divu[i+j*N] = u[i+1+j*Np1]-u[i+j*Np1] + u[Y+i+(j+1)*N]-u[Y+i+j*N]; }
    //cout << "div u:\n"; print(divu, N, N);
    //FORCC
    //    foo[i+j*N] -= p[i+j*N];
    //cout << "error:\n"; print(foo, N, N);
    ...
    //    double l=i>1?-1:0, r=i<N-2?-1:0, b=j>1?-1:0, t=j<N-2?-1:0;
    //    double c=-(l+r+b+t);
    //    foo[i+j*N] = c*p[i+j*N]+l*p[i-1+j*N]+r*p[i+1+j*N]+b*p[i+j*N-N]+t*p[i+j*N+N] - divu[i+j*N];
    //}
    //cout << "residual:\n"; print(foo, N, N);
    // /DEBUG
}
...
void updateAndAdvect(double flip) {
    for (int k=0; k<PN; ++k) {
        ...
        px[k] = min(max(px[k]+vx*dt,1.001),N-1.001);
        py[k] = min(max(py[k]+vy*dt,1.001),N-1.001);
    }
}
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        //for (int j=1; j<N; ++j)
        //    for (int i=1; i<N-1; ++i)
        //        if (flags[i+j*N] == FLUID || flags[i+j*N-N] == FLUID)
        //            u[Y+i+j*N] += -9.81*dt/(N-2);
